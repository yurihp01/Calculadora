...
class ConfiguracaoActivity: AppCompatActivity() {
    ...
    fun onClickSalvaConfiguracao(v: View) {
        ...
        saveOnSharedPreferences(novaConfiguracao)
    ...
    private fun saveOnSharedPreferences(configuration: Configuracao) {
        CalculatorPreferences.getPreferences(this)

        CalculatorPreferences.write(
            CalculatorPreferences.LAYOUT_KEY, configuration.leiauteAvancado
        )
        CalculatorPreferences.write(
            CalculatorPreferences.SEPARATOR_TYPE_KEY, configuration.separador.name
        )
    }
}